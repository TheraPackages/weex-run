...
module.exports = class DevicePanel {
  activate (state) {
    ...
    atom.commands.add(atom.commands.rootNode, commandProjectPathChanged, () =>  this.initDevicePanel())
  }
  ...
  frontCurrentDevice () {
    ...
    realDeviceInfo.getHistoryRealDeviceArray().forEach(function (actionView, udid, map) {
      if (actionView && actionView.tr) {
        var row = $(actionView.tr)
        var row1 = $(actionView.tr).closest('tbody').children().first()
        row.insertAfter(row1)
