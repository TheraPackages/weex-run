...
module.exports = class SimulatorManager {
  ...
  stop () {
    ...
    configurationManager.getLaunchConfig()
      .then((configObject) => {
        childProcess.exec(`xcrun simctl terminate ${_this.bootedUDID} ${configObject.appid}`);
      })
  }
  ...
  _curDeviceID () {
    let _this = this
    return new Promise((resolve, reject) => {
      _this._getSimulators().then((simulators) => {
        let avaliables = _this._availableSimulator(simulators)
        let defaultDevice = _this._defaultDevice(avaliables)
