...
WsBridge.Dispatcher.prototype = {
  ...
  _dispatchUtf8 (strData) {
    ...
    } else if(dataObj.message === 'poolSize-change'){
      atom.commands.dispatch(atom.views.getView(atom.workspace), 'dumpling-server:client-changed', dataObj)
    } else {
...
WsBridge.Collector = function (bridge) {
  ...
  this._subscriptions.add(atom.commands.add('atom-workspace', {
    ...
    // Tell connected devices to recreate current template.
    'thera-builder:push': (event) => this.builderSend(event.detail),
  }))
}
