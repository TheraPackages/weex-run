...
module.exports = class SimulatorActionView {
    ...
    setRunIcon(isStarted){
        ...
        if(isStarted){
            $(this.runIcon).attr('class','fa fa-stop weex_run_stop_icon weex_run_button_cursor')
            simulatorInfo.pushToStartedSimulator(this)
        } else {
            $(this.runIcon).attr('class','fa fa-play weex_run_start_icon weex_run_button_cursor')
            simulatorInfo.popFromStartedSimulator(this)
        }
    }
