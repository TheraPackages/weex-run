...
module.exports = class WeexRuner {
  ...
  _startPreviewFile () {
    return new Promise((resolve, reject) => {
      ...
      this.watchFilePath = undefined
      fs.readJson(lanuchConfigPath, (err, configObj) => {
        if (err) console.warn(`launch.json cannot find on path ${lanuchConfigPath}`)
        else {
          try {
            this.watchFilePath = configObj.main.replace('${workspaceRoot}', projectPath)
          } catch (e) {
            console.warn('cannot find entrance in config', e)
          }
        }
        resolve(this.watchFilePath)
      })
    })
  }
