...
module.exports = class SimulatorManagerUtil {
  ...
  devicesList () {
    const _this = this
    let pIosDevices = this._getSimulators()
    .then((simulators) => {
      ...
      childProcess.exec('xcrun --version', (error, stdout, stderr) => {
        ...
        if (error) {
          ...
          childProcess.exec('/Applications/Xcode.app/Contents/Developer/usr/bin/simctl list -j', (error, stdout, stderr) => {
            if (error) {
              console.warn(stderr)
              reject(new Error('Get simulator list failed, make sure /Applications/Xcode.app installed.'))
            } else {
  ...
  _listAndroidEmulators () {
    return new Promise((resolve, reject) => {
      childProcess.exec('command -v emulator && command -v adb', (error, stdout, stderr) => {
        if (error) {
          console.warn("Android dev environment hasn't installed or customed .profile/.bash_profile did not sourced.\n", error, stdout, stderr)
          resolve([])
        } else {
          doGet()
        }
    ...
    return new Promise(function (resolve, reject) {
      if (self.android.actEmuName === emulatorName) {
        ...
        function doKill() {
          ...
          childProcess.exec(`adb emu kill`, (error, stdout, stderr) => {
            if (error) {
  ...
  installAndroidApp (apkName) {
    return new Promise(function (resolve, reject) {
      childProcess.exec(`adb -e install -r -t -g ${apkName}`,
        (error, stdout, stderr) => {
          if (error) {
  ...
  notifyAndroidServerAddress (name, hostPort) {
    return new Promise(function (resolve, reject) {
      ...
      childProcess.exec(`echo '${configs}' > ${cacheFile} && adb -e root && adb -e push ${cacheFile} /data/data/com.alibaba.falcon/files/android.conf`,
        (error, stdout, stderr) => {
          if (error) {
