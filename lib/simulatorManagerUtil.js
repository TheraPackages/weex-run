...
module.exports = class SimulatorManagerUtil {
      ...
      if (type === GENERIC_DEVICE) {
        ...
        configurationManager.getConfig()
          .then((config) => {
            ...
            return _this.notifyAndroidServerAddress(`${ip.address()}:${atom.config.get('weex-run.dumplingPort')}`)
  ...
  notifyAndroidServerAddress (hostPort) {
    return new Promise(function (resolve, reject) {
      ...
      childProcess.exec(`echo '${configs}' > ${cacheFile} && adb -e push ${cacheFile} /data/data/com.alibaba.falcon/files/android.conf`,
        (error, stdout, stderr) => {
          if (error) {
